  ...
  const methods = {
      ...
      return new Promise((resolve, reject) => {
        ...
        initData(crud.url, crud.getQueryParams())
          .then((data) => {
            ...
            crud.page.total = data.count;
            crud.data = data.data;
            crud.resetDataStatus();
            setTimeout(() => {
              crud.loading = false;
              crud.selections = [];
              crud.selectedRowKeys = [];
    ...
    doDelete(data) {
      ...
      return crud.crudMethod
        .del(!crud.isBuildDeletePayload ? ids.join(',') : crud.crudMethod.buildDeletePayload(data))
        .then((resp) => {
          const { status, msg } = resp;
          if (status !== 0) {
            crud.notify(msg, CRUD.NOTIFICATION_TYPE.ERROR);
    ...
    selectionChangeHandler(selectionKeys, val) {
      crud.selectedRowKeys = selectionKeys;
      crud.selections = val;
    },
    ...
    resetForm(data) {
      ...
      for (const _key in form) {
        if (Object.prototype.hasOwnProperty.call(crudFrom, _key)) {
          crudFrom[_key] = form[_key];
        } else {
          Vue.set(crudFrom, _key, form[_key]);
        }
      }
      if (crud.findVM('form').$refs['form']) {
        crud.findVM('form').$refs['form'].clearValidate();
    ...
    toggleRowSelection(selection, data) {
      if (data.children) {
        data.children.forEach((val) => {
          selection.splice(
            selection.findIndex((item) => this.getDataId(item) === this.getDataId(val)),
            1,
          );
...
function mergeOptions(src, opts) {
  ...
  for (const _key in src) {
    if (Object.prototype.hasOwnProperty.call(opts, _key)) {
      optsRet[_key] = opts[_key];
    }
  }
  return optsRet;
}
...
function presenter(crud) {
  ...
  return {
    ...
    created() {
      for (const _k in this.$crud) {
        if (this.$crud[_k].queryOnPresenterCreated) {
          this.$crud[_k].toQuery();
        }
      }
    },
    destroyed() {
      for (const _k in this.$crud) {
        this.$crud[_k].unregisterVM('presenter', this);
      }
    },
    mounted() {
